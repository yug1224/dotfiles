...
| `suggest-plan.md`           | `/suggest-plan`           | Development | 要件から設計・実装計画を Advisor で策定し松竹梅で提案 |
...
開発ライフサイクルのフェーズに特化したアドバイザー。すべて `readonly: true`・`model: inherit` で、分析・提案のみを行う。
...
`/diff-review` と `/pr-review` はどちらも `quality-advisor` を並列起動してチェックリストベースの体系的レビューを行う。`/magi-pr-review` は MAGI ペルソナによる多角的評価を重視し、重要度の高い PR やアーキテクチャ変更を伴う PR に適する。
