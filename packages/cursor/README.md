...
| `fix-issue.md`               | `/fix-issue`               | Development | Notion 課題の原因調査と対応方針の提案                  |
...
| `diff-review.md`             | `/diff-review`             | Quality     | ステージング済み変更のセルフレビュー（出力前に再検証） |
| `pr-review.md`               | `/pr-review`               | Quality     | GitHub PR のレビュー（出力前に再検証・過剰指摘抑制）   |
| `magi-pr-review.md`          | `/magi-pr-review`          | Quality     | MAGI 3体合議 PR レビュー（出力前に再検証）             |
| `blog-plan.md`               | `/blog-plan`               | Writing     | テーマと概要からブログ記事の執筆プランを作成           |
| `blog-review.md`             | `/blog-review`             | Writing     | ブログ記事を 7 観点で評価                              |
| `graphic-record-prompt.md`   | `/graphic-record-prompt`   | Visual      | PR・ADR・開発ログから画像生成 LLM 向けプロンプトを出力 |

ラッパーは frontmatter のみ。本文は `@~/.config/shared/ai/commands/<name>.md` で取り込む（[CONVENTIONS.md](../shared/shared/ai/CONVENTIONS.md)）。

**出力再検証**: 手順の正本は `@~/.config/shared/ai/rules/conventions/output-verification-rule.md`。`/verify-output` は直前応答の単体再検証用。`suggest-plan` / `diff-review` 等は最終 Step で同ルールのインライン再検証を参照する（レビュー系は過剰指摘の削減を優先）。
...
1. 本文を `packages/shared/shared/ai/commands/<name>.md` に追加（frontmatter なし）
...
| `/diff-review`, `/pr-review`（サブエージェント明示時） | `quality-advisor`                 | Git 管理。明示がなければ親エージェントが直接分析                                                              |
...
1. 本文（frontmatter なし）を `packages/shared/shared/ai/rules/<subdir>/` に追加する
...
| `preToolUse`           | `rtk hook cursor`      | Shell 実行前に CLI を `rtk` 経由へ書き換え（トークン削減。[RTK](https://github.com/rtk-ai/rtk)） |

`preToolUse` は Homebrew の `rtk` バイナリを PATH から呼ぶ（`packages/cursor/hooks/` 配下のスクリプトではない）。初回確認は **`rtk init -g --no-patch --agent cursor`**（設定 JSON はパッチしない）。`stow` で `hooks.json` を上書きするため、正本は本リポジトリの [`hooks.json`](hooks.json) に `preToolUse` を含めること。RTK 利用ガイドの正本は [`packages/shared/shared/ai/docs/RTK.md`](../shared/shared/ai/docs/RTK.md)。**インストール確認**: `rtk --version && rtk gain`（詳細は [CONVENTIONS.md](../shared/shared/ai/CONVENTIONS.md) の RTK 節）。

`hooks/guard-shell.sh` は薄いラッパーであり、**判定の正本**は `make stow` 後の `~/.config/shared/ai/hooks/guard-shell.sh`（リポジトリでは [`packages/shared/shared/ai/hooks/guard-shell.sh`](../shared/shared/ai/hooks/guard-shell.sh)）に `exec` 委譲する。Claude Code 側も同じ共有スクリプトを adapter 経由で利用する。guard は `rtk ` プレフィックスを strip して分類する。
...
~/.cursor/
├── commands/
│   ├── diff-review.md                  ← Git 管理（ラッパー → shared 本文）
│   ├── pr-review.md                    ← Git 管理
...
`~/.cursor/hooks.json` が **symlink ではない通常ファイル**として存在する場合（`rtk init -g --agent cursor` 等で生成された場合）、`make stow` が失敗する。正本は本リポジトリの [`hooks.json`](hooks.json) である。

```bash
# 内容確認（repo と同一ならバックアップして削除してよい）
diff ~/.cursor/hooks.json packages/cursor/hooks.json
mv ~/.cursor/hooks.json ~/.cursor/hooks.json.bak
make stow
```

`rtk init` で設定をパッチしないよう、初回確認は **`rtk init -g --no-patch --agent cursor`** を使う（[CONVENTIONS.md](../shared/shared/ai/CONVENTIONS.md) の RTK 節）。
