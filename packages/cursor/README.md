...
- **`terminalAllowlist`**: 読み取り系コマンドのみを個別に許可する（Allowlist-first）。git/gh は安全な読み取り系コマンドのみ個別指定し、書き込み・ネットワーク系は Cursor の ask で処理する。プレフィックスマッチ（`pnpm` → `pnpm run lint`, `pnpm test` 等すべてにマッチ）
...
| カテゴリ             | エントリ                                                                                                                                                                                              | 備考                   |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| Unix ユーティリティ  | `awk`, `cat`, `cd`, `comm`, `cp`, `curl`, `cut`, `diff`, `echo`, `env`, `find`, `grep`, `head`, `kill`, `ls`, `mkdir`, `node`, `npx`, `paste`, `read`, `sed`, `sort`, `tail`, `timeout`, `uniq`, `wc` | 個別指定               |
| パッケージマネージャ | `pnpm`                                                                                                                                                                                                | 全サブコマンドをカバー |
