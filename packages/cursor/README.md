...
Cursor の設定ファイル群。`make mise`（ルート `mise.toml` の `[dotfiles]`）で `~/.cursor/` にデプロイされる。

共有の rules / commands 本文は `make mise` で `~/.config/shared/ai/` に展開される。`.mdc` やコマンドからは `@~/.config/shared/ai/...` 絶対パスで取り込む（例: `@~/.config/shared/ai/rules/...`、`@~/.config/shared/ai/commands/...`）。
...
この dotfiles で定義し、`make mise` で `~/.cursor/` にデプロイされるフック。

| イベント               | コマンド / スクリプト  | 説明                                                        |
| ---------------------- | ---------------------- | ----------------------------------------------------------- |
| `beforeShellExecution` | `hooks/guard-shell.sh` | 破壊的 git のブロック、`gh` / `pnpm exec`・`dlx` の確認促進 |
| `preToolUse`           | `./hooks/rtk-hook.sh`  | Shell を `rtk` 経由へ書き換え（matcher: `Shell`）           |
...
Cursor の Auto-run 時に承認なしで実行を許可するコマンド・MCP ツールの Allowlist 定義。`make mise` で `~/.cursor/permissions.json` にデプロイされると、Cursor Settings の対応する Allowlist は読み取り専用になりこのファイルの内容で上書きされる。
...
| `codegraph` / `user-codegraph` | `*`（現状 `codegraph_explore`）                                                           | ローカル知識グラフ。Cursor の `user-` プレフィックスと Claude の `mcp__codegraph__*` / `mcp__user-codegraph__*` を対称に。セットアップは [CODEGRAPH.md](../shared/ai/docs/CODEGRAPH.md) |
...
本パッケージ（`packages/cursor/`）が**正本（Source of Truth）**であり、`make mise` で `~/.cursor/` にデプロイされる。ローカル専用のコマンド・ルールは `*.local.md` / `*.local.mdc` の命名規則を使用する。`.gitignore` に `*.local.*` を定義しているため、ローカルファイルは Git 管理外となる。
...
[`packages/shared/ai/rules/conventions/review-common-rule.md`](../shared/ai/rules/conventions/review-common-rule.md) は同ディレクトリの **`./pr-review-rule.md`**（必須）と、存在する場合のみ同名の **`.local.md`** を参照する（運用は `shared/ai/README.local.md`、gitignore）。`make mise` で `~/.config/shared/ai/rules/conventions/` に展開する。
