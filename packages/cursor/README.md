...
| **hooks**    | エージェントの操作を監視・制御するガード                     | イベント駆動（自動実行）                                          |
...
> **ローカルコマンド**: 上記は Git 管理されている汎用コマンドのみ。`*.local.md` 形式のローカル専用コマンド（`/diff-review`, `/pr-review`, `/suggest-pr-description` 等）は Git 管理外のため一覧に含まない。詳細は「[ローカル拡張](#ローカル拡張)」セクションを参照。
...
3. ルールを参照する場合は `~/.cursor/rules/<subdir>/` の絶対パスで記述する
4. 上記コマンド一覧を更新する
...
| `/diff-review`, `/pr-review`（サブエージェント明示時） | `quality-advisor`                 | ローカル（`*.local.md`）。明示がなければ親エージェントが直接分析                                              |

> `/magi` は MAGI 3体を起動するコマンドであり、Advisor は使用しない。`/magi-pr-review` も MAGI 3体による PR レビューであり、Advisor は使用しない。いずれも詳細は下記の「MAGI ユニット（3体）」セクションを参照。
...
2. 適切なサブディレクトリに配置する（`advisor/`, `checklists/`, `conventions/` 等）
3. ファイル名は `<name>-rule.mdc`、チェックリストは `<name>-checklist.mdc`、ローカル専用は `<name>.local.mdc` とする
4. コマンドやエージェントから参照する場合は `~/.cursor/rules/<subdir>/` の絶対パスで記述する
5. 上記ルール一覧を更新する
...
この dotfiles で定義し、`stow` で `~/.cursor/` にデプロイされるフック。

| イベント               | スクリプト             | 説明                                                 |
| ---------------------- | ---------------------- | ---------------------------------------------------- |
| `beforeShellExecution` | `hooks/guard-shell.sh` | 破壊的な git コマンドのブロック、`gh` CLI の使用促進 |

`hooks/guard-shell.sh` は薄いラッパーであり、**判定の正本**は `make stow` 後の `~/.config/shared/ai/hooks/guard-shell.sh`（リポジトリでは [`packages/shared/shared/ai/hooks/guard-shell.sh`](../shared/shared/ai/hooks/guard-shell.sh)）に `exec` 委譲する。Claude Code 側も同じ共有スクリプトを adapter 経由で利用する。

#### guard-shell.sh の判定ロジック

`hooks.json` の `matcher: "git |gh "` により、コマンド文字列に `git ` または `gh ` を含む場合のみフックが起動する。`failClosed: true` により、スクリプトの異常終了時もコマンドをブロックする（fail-closed）。`jq` が未インストールの場合や入力の解析に失敗した場合もセキュリティ判定が行えないため deny を返す（fail-closed）。フックの役割は **deny と ask**。破壊的コマンドを完全にブロック（deny）し、確認が望ましいコマンドにはユーザー承認を要求（ask）する。それ以外は allow を返し、Allowlist（`permissions.json`）と Cursor が制御する。以下の判定テーブルは git / gh の全サブコマンドを網羅するものではなく、パターンに該当しないコマンドは allow としてフックを通過する点に留意すること。

代表ケースの自動検証: [`packages/shared/shared/ai/hooks/guard-shell.test.sh`](../shared/shared/ai/hooks/guard-shell.test.sh)（`jq` 必須）。
...
| 上記以外                                                                     | allow | フックは通過。Allowlist / Cursor が制御         |
...
Cursor の Auto-run 時に承認なしで実行を許可するコマンド・MCP ツールの Allowlist 定義。`stow` で `~/.cursor/permissions.json` にデプロイされると、Cursor Settings の対応する Allowlist は読み取り専用になりこのファイルの内容で上書きされる。

### 設計方針

- **`terminalAllowlist`**: 読み取り系コマンドのみを個別に許可する（Allowlist-first）。git/gh は安全な読み取り系コマンドのみ個別指定し、書き込み・ネットワーク系は Cursor の ask で処理する。プレフィックスマッチ（`pnpm run` → `pnpm run lint`, `pnpm run test` 等にマッチ）。`curl`・`npx`・`pnpm exec`・`pnpm dlx` など任意コマンド実行やネットワークアクセスが可能なコマンドは除外し、Cursor の ask で制御する
- **`mcpAllowlist`**: 読み取り系パターンのみ許可。書き込み系ツール（`create_*`, `update_*`, `use_*` 等）は含めない。`server:tool` 形式、glob パターン対応（`github:get_*` → `get_commit`, `get_file_contents` 等にマッチ）
...
| パッケージマネージャ | `pnpm add`, `pnpm audit`, `pnpm install`, `pnpm list`, `pnpm ls`, `pnpm outdated`, `pnpm remove`, `pnpm run`, `pnpm store`, `pnpm test`, `pnpm why`                                    | サブコマンド単位で指定。`pnpm exec` / `pnpm dlx`（任意実行）は除外し Cursor が制御 |
...
| `notion` / `user-notion` | `notion-fetch`, `get_page`, `read_page`, `fetch`, `retrieve_page`, `notion-search`, `notion-get-comments` | 個別指定                           |
...
本パッケージ（`packages/cursor/`）が**正本（Source of Truth）**であり、`stow` で `~/.cursor/` にデプロイされる。ローカル専用のコマンド・ルールは `*.local.md` / `*.local.mdc` の命名規則を使用する。`.gitignore` に `*.local.*` を定義しているため、ローカルファイルは Git 管理外となる。

```
~/.cursor/
├── commands/
│   ├── magi.md                         ← Git 管理（stow デプロイ）
│   ├── suggest-plan.md                 ← Git 管理（stow デプロイ）
│   ├── diff-review.local.md            ← ローカル専用
│   └── pr-review.local.md              ← ローカル専用
├── rules/
│   ├── checklists/
│   │   └── code-review-checklist.mdc   ← Git 管理
│   └── conventions/
│       ├── branch-name-rule.mdc        ← Git 管理
│       └── coding-rule.local.mdc       ← ローカル専用
└── ...
```

| 区分     | ファイル名                   | 管理形態        | 用途                                             |
| -------- | ---------------------------- | --------------- | ------------------------------------------------ |
| 正本     | `*.md` / `*.mdc`             | Git 管理 + stow | 汎用的なコマンド・ルール。チーム共有可能         |
| ローカル | `*.local.md` / `*.local.mdc` | Git 管理外      | 機密情報を含むもの、端末固有の設定、実験的なもの |

### 共有ルール向けローカル補助ファイル（`review-common-rule`）

[`packages/shared/shared/ai/rules/conventions/review-common-rule.md`](../shared/shared/ai/rules/conventions/review-common-rule.md) は、同じディレクトリにある **`./pr-review-rule.local.md`**（レビュー基準・重要度・テンプレート）を参照する。これは **共有ツリー側**のローカルファイルであり、`~/.cursor/rules/` ではなく次に置く。

- **推奨**: `packages/shared/shared/ai/rules/conventions/pr-review-rule.local.md` を作成する（`*.local.*` は `.gitignore` で Git 管理外）。`make stow` により `~/.config/shared/ai/rules/conventions/` に展開され、Cursor / Claude のどちらから `review-common` を読んでも参照できる。
- ホーム直下だけで管理したい場合は、`~/.config/shared/ai/rules/conventions/pr-review-rule.local.md` でもよいが、dotfiles と二重管理になりやすい。
...
| コマンド          | 対象                 | 用途                                 | Advisor                     |
| ----------------- | -------------------- | ------------------------------------ | --------------------------- |
| `/diff-review`    | ステージング済み差分 | PR 作成前のローカルセルフレビュー    | `quality-advisor`（明示時） |
| `/pr-review`      | GitHub PR URL        | リモート PR のレビュー               | `quality-advisor`（明示時） |
| `/magi-pr-review` | GitHub PR URL        | MAGI 3体合議による多角的 PR レビュー | MAGI 3体                    |

`/diff-review` と `/pr-review` はサブエージェント利用を明示した場合に `quality-advisor` を並列起動し、チェックリストベースの体系的レビューを行う。明示がない場合は親エージェントが同じ観点で直接調査する。`/magi-pr-review` は MAGI ペルソナによる多角的評価を重視し、重要度の高い PR やアーキテクチャ変更を伴う PR に適する。
...
デプロイ後のパスは `~/.cursor/` になるため、コマンドやエージェント内でのルール参照は `~/.cursor/rules/<subdir>/...` の絶対パスを使用する。
