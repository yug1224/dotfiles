...
開発ライフサイクルのフェーズに特化したアドバイザー。すべて `readonly: true`・`model: inherit` で、分析・提案のみを行う。ユーザーがサブエージェント利用を明示した場合、またはサブエージェントを使うコマンドから起動された場合にのみ使用される（description match による自動委譲は行わない）。
...
| `/diff-review`, `/pr-review`              | `quality-advisor`                 | ローカル（`*.local.md`）。詳細は「[ローカル拡張](#ローカル拡張)」参照                                         |
...
代表ケースの自動検証: `hooks/guard-shell.test.sh` を実行する（`jq` 必須）。
...
| コマンド          | 対象                 | 用途                                 | Advisor           |
| ----------------- | -------------------- | ------------------------------------ | ----------------- |
| `/diff-review`    | ステージング済み差分 | PR 作成前のローカルセルフレビュー    | `quality-advisor` |
| `/pr-review`      | GitHub PR URL        | リモート PR のレビュー               | `quality-advisor` |
| `/magi-pr-review` | GitHub PR URL        | MAGI 3体合議による多角的 PR レビュー | MAGI 3体          |

`/diff-review` と `/pr-review` はどちらも `quality-advisor` をサブエージェントとして並列起動し、チェックリストベースの体系的レビューを行う（`quality-advisor` の description にある「サブエージェントを使うコマンドから起動」に該当）。`/magi-pr-review` は MAGI ペルソナによる多角的評価を重視し、重要度の高い PR やアーキテクチャ変更を伴う PR に適する。
