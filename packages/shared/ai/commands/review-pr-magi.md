...
- 指摘事項には必ず Priority（P0 / P1 / P2）を付与すること
...
- **P0** (Critical): セキュリティ脆弱性、データ損失、クラッシュ、ビルドエラー、重大なアーキテクチャ違反。マージ前に必須修正。
- **P1** (High): バグ、誤動作、エッジケース、仕様不一致、パフォーマンス問題、型の安全性欠如。マージ前に修正すべき。
- **P2** (Medium): コード品質、保守性、ベストプラクティス、可読性の懸念。検討して改善（Nice to have）。
- スタイル・好みのみの nit は **出さない**。確認事項は短く書くか省略する。
...
- P0 または P1 の指摘がある場合: REJECT
- P2 のみで P0 / P1 がない場合: CONDITIONAL
- 指摘なしの場合: APPROVE
...
- **P0-1. {問題タイトル}**（指摘元: {ユニット名}）
  - **ファイル**: `{ファイルパス}` / **現状**: {問題点} / **理由**: {理由} / **推奨**: {修正案}

#### 🟠 P1

- **P1-1. {問題タイトル}**（指摘元: {ユニット名}）
...
- **Must have**: P0 / P1 を修正する（マージブロッカー）
- **Nice to have**: P2 の改善推奨事項（別タスクとして対応可）
