GitHub PR・ADR・開発ログ・自由記述を入力に、Gemini Nano Banana Pro へ渡す **構造専用の固定指示 + 日本語描画内容**（画風・トンマナは別メッセージ）を 1 本だけ出力する。画像は生成しない（下流の別チャットで生成する）。
...
**Input**: `/write-graphic-prompt` の後ろ。PR URL、`.md` パス、自由記述、複数可。
...
- `/write-graphic-prompt https://github.com/owner/repo/pull/123`
- `/write-graphic-prompt size:2560x1440 ./docs/adr/0001-use-postgres.md`
- `/write-graphic-prompt 認証を JWT+Refresh に変更。背景はセッション Cookie のみ。目標はリフレッシュ対応。`
...
応答冒頭に `✅️: /write-graphic-prompt` のみ（ルール側の `✅️: graphic-record-*` はユーザー向けに出さない）。
...
スタイルルールの **ブロックA固定文案（構造専用・情報密度必須）・3まとまり必須+残課題任意・実装が最大** を正とする。**`image_generation_prompt` は使う。ADR型 `compressed_summary` の出力・ネスト markdown は使わない**（`compressed_summary` は Step 3 内部 QA のみ）。**縦4帯・12セクション・確認用の別出力は使わない**。
...
スタイルルール「ブロックA固定文案」を **そのまま**出力し、`{image_generation_prompt}` のみ差し替える。`size:` 指定時は 【レイアウト】の解像度行（`1920x1080`）を置換。
...
- [ ] `✅️:` + 導線文 + **fenced ブロック 1 つだけ**（画像生成用）
- [ ] ブロックA固定文 4 セクション（出力指示・レイアウト・描画内容・避ける要素）+ `image_generation_prompt` のみ（`compressed_summary`・ネスト markdown・別ブロックなし）
- [ ] `image_generation_prompt` は **全文日本語**、**2000〜2800 字**、まとまり見出し `【1.背景】`〜`【3.実装】`（`【4.残課題】` は任意）
- [ ] 画像内固定文言が **「」** で囲まれている（タイトル・キャプション・コールアウト・熊のセリフ）
- [ ] 各列に **`列リード:`（50〜90字）** がある
- [ ] 背景・目標に **ミニ図解が各 3 項目**
- [ ] 各キャプションが **名詞+説明文（70〜110字/項目）**
- [ ] 実装列に **小見出し帯（`■`）・表・フロー・黄コールアウト（2つ以上）** がある
- [ ] 【レイアウト】にまとまり（背景/目標/実装、残課題は任意）・実装が最大・左下熊・情報密度の指示がある
- [ ] 【レイアウト】・【避ける要素】に画風・トーン・質感・背景色・左→右の流れ指定が **含まれていない**
...
`✅️:` の次に、次の 1 行だけを出力する（言い換え可）。

「Gemini で先に画風・トンマナを別メッセージで指示し、続けて以下の fenced ブロック（構造・描画内容）を貼り付けて画像を生成してください。」
...
- GenerateImage / 画像 API は呼ばない（2 段階ワークフロー: Step 1=本コマンドで構造ブロック出力、Step 2=下流で画風を別メッセージ → 構造ブロックを貼付）
- 情報不足時は推測せず 1〜2 点確認
- 応答は **fenced ブロック 1 つ + 導線文** のみ。ブロック外にメタ・ラベル一覧・要点箇条書き・`compressed_summary`・確認用の第二ブロックを出さない
- 画風・トンマナ・色・質感は **fenced ブロックに含めない**（下流で別メッセージ）。ブロックAは構造・描画内容のみ
