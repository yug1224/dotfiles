...
**参照ルール**: `packages/shared/ai/rules/conventions/branch-name-rule.md`（make mise 後 `~/.config/shared/ai/rules/` 等。Cursor ラッパー: `branch-name-rule.mdc`。所在は `shared/README.md` の「ルールファイルの所在」）
...
`packages/shared/ai/rules/conventions/branch-name-rule.md`（または make mise 後の `~/.config/shared/ai/rules/conventions/branch-name-rule.md`）を Read ツールで読み込み、ブランチ名規約を把握する。
