...
- 詳細チェックリスト: `packages/shared/ai/rules/checklists/code-review-checklist.md`（mise-dotfiles 後 `~/.config/shared/ai/rules/` 等。Cursor ラッパー: `code-review-checklist.mdc`）
...
- マルチテナント環境でのデータ分離
- 詳細チェックリスト: `packages/shared/ai/rules/checklists/owasp-top10-checklist.md`（mise-dotfiles 後 `~/.config/shared/ai/rules/` 等。Cursor ラッパー: `owasp-top10-checklist.mdc`）
...
- 詳細チェックリスト: `packages/shared/ai/rules/checklists/test-strategy-matrix.md`（mise-dotfiles 後 `~/.config/shared/ai/rules/` 等。Cursor ラッパー: `test-strategy-matrix.mdc`）
...
4. 重要度を明確にする（CRITICAL / HIGH / MEDIUM / LOW を区別 — 定義は `review-common-rule.md` / `pr-review-rule.md` に準拠）
