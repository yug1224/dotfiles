...
発見された問題を **P0 / P1 / P2** の 3 段階（Priority）に分類する。各指摘にプレフィックス付き連番を振る（P0-1, P0-2, P1-1, P2-1 等）。
...
- スタイル・好み・表記揺れのみの指摘は **原則出さない**（Greptile の nit 抑制に合わせる）
- 保守・可読に実害がある場合のみ **P2** とする
- 判断が難しい場合は、確認事項を本文に短く書くか省略する（旧 LOW 相当の独立カテゴリは設けない）
...
- **重要度別の指摘**: 行番号付き（`file.ts:42` 形式）で記載（P0 / P1 / P2）
- **良い点**: 評価できるポイントの記載
- **集計**: Priority 別の件数
- **次のアクション**: Must have（P0 / P1）/ Nice to have（P2）の2段階
...
- P0 / P1 の指摘には具体的な修正コード例を添える
- 良い実装や工夫が見られる箇所は積極的に「良い点」として評価する
- スタイル・好みのみの nit は出力しない。判断困難な点は確認事項として短く書くか省略する
- 指摘がないカテゴリも「_該当なし_」として明示する
...
- 扱い: Severity Badges の優先度哲学のみを蒸留・再構成した規範である（Confidence Score 等の製品固有 UI は含まない）
