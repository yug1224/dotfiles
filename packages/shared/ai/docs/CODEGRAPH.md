...
| Cursor `mcpAllowlist`      | `codegraph:*`, `user-codegraph:*`（現状ツールは `codegraph_explore`）      |
| Claude `permissions.allow` | `Bash(codegraph:*)`, `mcp__codegraph__*`, `mcp__user-codegraph__*`（同上） |
