...
**Input**: `/blog-plan` の後に続くテキストは、テーマの説明・概要・補足情報（参考リンク、対象読者、伝えたい主張など）。既存プランファイルのパスを渡すと修正版を作成する。

**使用例**:

- `/blog-plan oxlint と oxfmt を導入して CI を高速化した話。Before/After の数値あり。対象読者はフロントエンドエンジニア。`
- `/blog-plan ./drafts/blog-plan-oxlint.md を修正してほしい。タイトルをもっとキャッチーに。`
- `/blog-plan EMからICへのキャリアチェンジ。登壇資料のブログ化。補足: https://speakerdeck.com/yug1224/xxx`
...
応答の冒頭に `Applied: /blog-plan` と出力する。
...
プランを提示し、修正したい箇所がないか確認する。修正がある場合は Step 3 に戻って反復する。

---

## Guardrails

- `/blog-plan` はプランの作成のみを行い、記事本文は書かない
