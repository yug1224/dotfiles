...
1. `@~/.config/shared/ai/rules/conventions/development-log-rule.md` を Read（必須）
2. `@~/.config/shared/ai/rules/conventions/ticket-retrieval-rule.md` を Read（必須）
3. 同ディレクトリの `ticket-retrieval-rule.local.md` を Glob。存在する場合のみ Read（矛盾時は `.local.md` を優先）
