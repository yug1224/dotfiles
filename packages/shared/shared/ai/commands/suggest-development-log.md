Notion タスクと GitHub PR の情報を収集・分析し、開発ログ記載ルール（MECE 構造: 概要 / 分析 / 方針 / 実装 / 検証 / 残課題）に基づいて開発ログを生成する。

**参照ルール**: `@~/.config/shared/ai/rules/conventions/development-log-rule.md`

**Input**: `/suggest-development-log` の後に続く引数は、Notion URL（必須）と GitHub PR URL（任意、複数可）。

**使用例**:

- `/suggest-development-log https://notion.so/xxx`
- `/suggest-development-log https://notion.so/xxx https://github.com/owner/repo/pull/123`
- `/suggest-development-log https://notion.so/xxx https://github.com/owner/repo/pull/123 https://github.com/owner/repo/pull/124`
...
応答の冒頭に `Applied: /suggest-development-log` と出力する。

### 1. ルールの読み込み

`@~/.config/shared/ai/rules/conventions/development-log-rule.md` を Read する。
...
Notion URL が未指定の場合はユーザーに入力を求める。
...
参照ルールの MECE テンプレートと記載量の目安に従い、以下の構成で出力する。
...
参照ルールの「開発ログのテンプレート」および「記載量の目安」に従い、Markdown 形式で出力する。
コードブロック内はそのままコピー可能にする。
...
- Notion URL が未指定の場合はユーザーに入力を求める
- GitHub URL が未指定でも Notion 情報だけで概要・分析・方針は生成可能とする
...
- Notion MCP は引数で URL が渡された場合のみ使用し、渡されなければ呼び出さない
