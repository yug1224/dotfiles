...
```bash
make brew           # Brewfile: rtk, jq 等（homebrew-core の brew 'rtk'）
make stow           # shared → ~/.config/shared/ai/
                    # cursor → ~/.cursor/（hooks.json 含む）
                    # claude → ~/.claude/（settings.json 含む）
make mise-dotfiles  # rtk → ~/.config/rtk/（mise [dotfiles]）
rtk --version && rtk gain   # smoke test（下記 §3）
```

| 依存                      | 用途                               |
| ------------------------- | ---------------------------------- |
| `rtk`（PATH）             | hook 書き換え・出力圧縮            |
| `jq`                      | guard-shell 判定                   |
| `make stow` 済み          | hook 設定（cursor / claude）の展開 |
| `make mise-dotfiles` 済み | `~/.config/rtk/config.toml` の展開 |
...
| レイヤ        | 正本                                                                | 役割                                                                            |
| ------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| **guard**     | `packages/shared/shared/ai/hooks/guard-shell.sh`                    | git / gh / pnpm の deny・ask。`rtk ` プレフィックスを strip して分類            |
...
Allowlist 同期手順: [AGENTS.md](../AGENTS.md)。検証: `packages/shared/scripts/check-allowlist-sync.sh`, `check-deny-guard-sync.sh`。
...
which rtk             # homebrew-core の rtk であること
```

**名前衝突**: `rtk gain` が失敗する場合、[reachingforthejack/rtk](https://github.com/reachingforthejack/rtk)（Rust Type Kit）が PATH に入っている可能性がある。`brew info rtk` で homebrew-core 由来を確認する。
...
| 項目               | 正本                            | デプロイ先                  |
| ------------------ | ------------------------------- | --------------------------- |
| RTK グローバル設定 | `packages/rtk/rtk/config.toml`  | `~/.config/rtk/config.toml` |
...
- **`rtk init -g` をそのまま流さない** — `settings.json` / `hooks.json` を直接パッチし stow と競合する
...
| ツール | 衝突ファイル                                  | 対処                                                          |
| ------ | --------------------------------------------- | ------------------------------------------------------------- |
| Claude | `~/.claude/settings.json`, `~/.claude/RTK.md` | repo と `diff` し、同一ならバックアップして削除 → `make stow` |
...
| RTK コマンド     | `rtk hook claude`                                                   | `rtk hook cursor`                                             |
