...
make stow           # ~/.config/mise（tools 設定）
make mise-dotfiles  # shared / rtk / cursor / claude 等
...
| `jq`                      | guard-shell 判定                                         |
| `make stow` 済み          | `~/.config/mise` の展開                                  |
| `make mise-dotfiles` 済み | shared / rtk / cursor（hooks）/ claude（settings）の展開 |
...
- 設定 JSON の正本は本リポジトリ（`make mise-dotfiles` 管理）
...
| ツール | 衝突ファイル                                  | 対処                                                                   |
| ------ | --------------------------------------------- | ---------------------------------------------------------------------- |
| Claude | `~/.claude/settings.json`, `~/.claude/RTK.md` | repo と `diff` し、同一ならバックアップして削除 → `make mise-dotfiles` |
