...
| 種別              | 配置                                                                  | 用途                                                      |
| ----------------- | --------------------------------------------------------------------- | --------------------------------------------------------- |
| Convention        | `rules/conventions/`                                                  | 行動規範（レビュー、CodeGraph、Applied、チケット取得）    |
...
1. 本文（frontmatter なし）を `packages/shared/shared/ai/rules/<subdir>/` に `.md` で追加する。1 行目に `Applied: <rule-id>`。
